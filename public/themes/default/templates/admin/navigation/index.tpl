...
{foreach from=$menu item=item nocache}
  ...
  {elseif $item.id == 2}
    ...
    {foreach from=$cmenu item=sitem}
      <li id="cl{$sitem.id}">{$sitem.name} &nbsp;&nbsp;<a href="{$view->url(['action' => 'delete', 'parent' => $sitem.parent, 'item' => $sitem.id])}" title="{'Delete'|translate}"><i class="icon-trash"></i></a></li>
    {/foreach}
    </ul>
  {else}
    <li id="nl{$item.id}"><a href="{$view->url(['action' => 'edit', 'id' => $item.id])}">{$item.name}</a> &nbsp;&nbsp;<a href="{$view->url(['action' => 'delete', 'item' => $item.id])}"><i class="icon-trash" title="{'Delete'|translate}"></i></a>
  {/if}
  </li>
{/foreach}
</ul>
