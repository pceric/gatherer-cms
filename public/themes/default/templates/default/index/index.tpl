...
{if !empty($sticky)}
		<div class="sticky">
{section name=sloop loop=$sticky}
			...
			<div class="meta">
				<p class="links">Tags: {$sticky[sloop].tags|tagify}{if !empty($sticky[sloop].comments)} | {include file="disqus_count.tpl" disqus_identifier="news_{$sticky[sloop].id}"} {'Comments'|translate}{/if} | <a href="{$view->url(['module' => 'news', 'type' => 'news', 'id' => $sticky[sloop].id],null,true)}">Permalink</a>{if $gcms.isAdmin == true} | <a href="{$view->url(['module' => 'news', 'controller' => 'admin', 'action' => 'edit', 'type' => 'news', 'id' => $sticky[sloop].id])}">{'Edit'|translate}</a>{/if}</p>
			</div>
{/section}
		</div>
{/if}
{* Now our regular posts *}
{section name=floop loop=$feed}
		{if $feed[floop].type == 'reader'}<div class="gfeed">{else}<div class="news">{/if}
			...
			<div class="meta">
				<p class="links">Tags: {$feed[floop].tags|tagify}{if !empty($feed[floop].comments)} | {include file="disqus_count.tpl" disqus_identifier="news_{$feed[floop].id}"}  {'Comments'|translate}{/if} | <a href="{$view->url(['module' => 'news', 'type' => $feed[floop].type, 'id' => $feed[floop].id],null,true)}">Permalink</a>{if $gcms.isAdmin == true && $feed[floop].type != 'reader'} | <a href="{$view->url(['module' => 'news', 'controller' => 'admin', 'action' => 'edit', 'type' => 'news', 'id' => $feed[floop].id])}">Edit</a>{/if}</p>
			</div>
		</div>
{/section}
